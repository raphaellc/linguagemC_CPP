...
void scan_port(const char *target_ip, int target_port) {
    ...
    // Socket para receber
    SOCKET recv_sock = socket(AF_INET, SOCK_RAW, IPPROTO_IP);
    ...
    while ((bytes_received = recvfrom(recv_sock, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&from, &fromlen)) > 0) {
        ...
        TCP_HDR *recv_tcph = (TCP_HDR *)(buffer + (recv_iph->ihl * 4));
    ...
    if (bytes_received > 0) {
        ...
        // Processar os dados recebidos em 'buffer'
    } else if (bytes_received == 0) {
        printf("Conexão fechada pelo par\n");
        // Tratar o fechamento da conexão (apenas para TCP)
    } else {
        int erro = WSAGetLastError();
        printf("Erro ao receber dados: %d\n", erro);
        perror("recvfrom"); // Imprime a mensagem de erro associada ao 'errno'
        // Tratar o erro (ex: encerrar o programa, tentar novamente)
    }
